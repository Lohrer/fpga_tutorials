...
\begin{tikzpicture}[
    %framed,scale=1, every node/.style={scale=1},
    box/.style={%
        rectangle, draw=black, very thick, minimum height=2.25em, minimum width=10.5em,%
        inner xsep=.25cm, outer sep=.10cm, text centered,
        draw, align=center
    },
    %%% Arrow styles
    arrow/.style={very thick, ->, >=stealth},
    dblarrow/.style={very thick, <->, >=stealth},
    componentnode/.style={shape=rectangle},
    memorynode/.style={shape=rectangle}
    ]

    %%% Components
    \node [box] (sampler) {\small Sample Collector};
    \node [below of=sampler, yshift=-.5cm, box] (features) {\small Feature Generator};

    %%% Arrows
    \draw [arrow] (sampler) -- (features);
\end{tikzpicture}
